...
app.post('/inreg', function(req, res){
  var dateFormular= new formidable.IncomingForm()
	dateFormular.parse(req, function(err, fields, files){
		...
		var textFisier= fs.readFileSync("./assets/json/useri.json") //cale relativa la index.js
		...
		if(fields.activitate){
			console.log(fields.activitate.length)
			sirActivitate=fields.activitate
		}
        else sirActivitate=""
    ...
    var jsonNou=JSON.stringify(objson);
		fs.writeFileSync("./assets/json/useri.json", jsonNou);
		res.redirect("/")
	});
